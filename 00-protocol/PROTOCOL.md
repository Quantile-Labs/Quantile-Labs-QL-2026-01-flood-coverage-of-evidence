...
| 2026-08-21 | §5 definition of *evidenced* | **POST-HOC SENSITIVITY, and it tests a claim the frozen text makes about itself. §5 says the frozen definition is "the most generous defensible reading" and that where a design choice could bias either way this study takes the option that flatters the subject. That claim was never checked on the period axis.** The frozen definition reads `google/2014/dual_lstm/full_run/` only. The release also publishes `google/1980/dual_lstm/full_run/`, which carries values for roughly twice as many African gauges. P_unevidenced is therefore recomputed with *evidenced* defined on the 1980 path, and on the union of both paths, and published beside the frozen figure. **The frozen definition is not replaced.** What is being measured is what it costs. | Raised by the second external reviewer on 2026-08-21 and verified before being acted on: 479 African gauges carry a derived F1 on the 1980 path against 230 on the 2014 path at the 1.01-year return period, and the ratio widens at rarer return periods. The 2026-08-04 amendment settled the *experiment* axis, `full_run` against `kfold_splits`, and concluded correctly that `full_run` is the generous reading. The *period* axis was recognised for Q4 on the same day, where 1980 runs alongside 2014, and was never carried across to the evidence definition. It fell between two amendments. The reviewer's indicative sizing is about 7 pp against a headline of 92.4%, which would be larger than every sensitivity this study has measured put together, and Gate D's abandonment threshold is 10 pp. A study that had this pointed out in writing and published without measuring it would deserve what followed. |
| 2026-08-21 | §5, §9 degenerate values | **BINDING CORRECTION, two parts. First, the degeneracy flag is one-sided and is replaced by two flags reported side by side.** `03_q4_skill.py` flagged a pair as degenerate only where precision *and* recall are both exactly 1.0, while the released code's degenerate return is per quantity, so a gauge degenerate in one and genuine in the other went unflagged. Both a strict flag, both equal to 1.0, and a broad flag, either equal to 1.0, are now computed and reported, and neither is designated the truth. **Second, the 2026-08-04 commitment to report degenerate counts at every return period, with Q4 both including and excluding them, is honoured in the Note and was not.** | The first is a defect found by review and fixed. The second is a live breach of a commitment this protocol already made: the draft reported degeneracy only as a fifty-year curiosity while it affects 17.5% of gauges at the five-year return period and 31% at the ten-year, moving unweighted F1 by 13.0 and 24.7 points. The amendment said that where the including and excluding figures disagree materially the disagreement is the result. They disagree materially, and reporting only the higher figure ran in the subject's favour, which is consistent with our stated bias policy and is not what was promised. A commitment kept only where it is convenient is not a commitment. |
| 2026-08-21 | §10 Gate F | **DEFECT IN A PRE-REGISTERED GATE, recorded rather than repaired in place. The two sides of Gate F are computed on different rungs of the denominator ladder, so the reported difference of 0.26 pp is an artifact of the mismatch.** The population side is rung 2b, all 5,734 basins holding any inventory point; the gauge side is rung 2, the 3,090 real gauges, as this protocol specified on 2026-08-07. Matched on rung 2 the difference is 2.712 pp, and matched on rung 2b it is 4.000 pp. **The verdict is unchanged: the null stands on both matchings, comfortably inside the 5 pp threshold.** The Note publishes all three figures and states which is which, and the pre-registered gate keeps its recorded verdict rather than being quietly recomputed. | The 2026-08-07 amendment reasoned at length about which rung the gauge comparator belongs on and ruled out rung 2b because the 3,682 undocumented entries guarantee a low gauge count for a reason unconnected to evidence. That argument applies with identical force to the population side, which is on rung 2b. The amendment recorded "Population-weighted coverage was 12.4% throughout", treating that side as fixed while the gauge rung varied, and the decision it needed was never made. Worth stating plainly because it cuts the other way as well: matched on rung 2, coverage is *better* where people are, 10.544% against 7.832%, which is a more interesting null than the one the draft reports and is further from the concern we pre-registered. Found by external review on 2026-08-21 and verified independently before being written down. |
